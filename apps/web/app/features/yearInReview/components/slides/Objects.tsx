import type { YearInReviewFace, YearInReviewObject } from '@shared/schemas/yearInReview'
...
export function Objects({ title, content, objects, faces }: Props) {
  const getDelay = (sectionIndex: number, itemIndex: number) => {
    const baseDelay = sectionIndex * 0.4
    return baseDelay + itemIndex * 0.05
  }
          ...
          <div className="flex flex-col gap-10 pb-10">
            {objects.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {objects.slice(0, 8).map((item, index) => (
                    <StatCard
                      key={`obj-${item.name}`}
                      label={item.name}
                      count={item.count}
                      delay={getDelay(0, index)}
                    />
                  ))}
                </div>
              </div>
            ...
            {faces.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {faces.slice(0, 8).map((item, index) => (
                    <StatCard
                      key={`face-${item.name}`}
                      label={item.name}
                      count={item.count}
                      delay={getDelay(1, index)}
                    />
                  ))}
                </div>
              </div>
