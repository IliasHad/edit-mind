...
export function Categories({ title, content }: Props) {
  ...
  useEffect(() => {
    const categories = Array.from(content.matchAll(/([\w\s]+):\s*(\d+)%/g)).map(([_, name, percent], index) => ({
      name: name.trim(),
          ...
          <div className="w-full max-w-3xl space-y-5 max-h-[60vh] overflow-y-auto custom-scrollbar pr-4">
            ...
            >
              <div className="flex items-start gap-4">
                <p className="text-lg font-medium text-white/90 leading-relaxed tracking-tight">{content}</p>
              </div>
            </motion.div>
          </div>
        ) : (
